...
namespace LoadBalancingBroker
{
    class Program
    {
        ...
        {
            int LBBroker_Clients = 10;
            int LBBroker_Workers = 3;

            if (args.Length == 2)
            {
                LBBroker_Clients = int.Parse(args[0]);
                LBBroker_Workers = int.Parse(args[1]);
            }
            ...
            using (var backend = new ZSocket(context, ZSocketType.ROUTER))
            {
                ...
                while (true)
                {
                    ...
                    {
                        incoming.DumpZmsg("---backend incoming---");
                    ...
                    if (worker_queue.Count > 0)
                    {
                        ...
                        if (frontend.PollIn(poll, out incoming, out error, TimeSpan.FromMilliseconds(64)))
                        {
                            ...
                            using (var outgoing = new ZMessage())
                            {
                                //here, custimize the message, add clientid as data.
                                //when worker recieved, there is just only three frame.
