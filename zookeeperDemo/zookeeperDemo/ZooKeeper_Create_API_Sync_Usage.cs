...
{
    class ZooKeeper_Create_API_Sync_Usage : IWatcher
    {
        static ManualResetEvent manualWaitHandler = new ManualResetEvent(false);//false 即非终止，未触发。  

        static void Main(string[] args)
        {
            ZooKeeper zookeeper = new ZooKeeper("127.0.0.1:2181", new TimeSpan(0,0,5), new ZooKeeper_Create_API_Sync_Usage());
            try
            {
                manualWaitHandler.WaitOne();

                //there is no Async Create API for .net client for now 
